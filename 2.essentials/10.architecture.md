...
---
type: "warning"
label: "Avoid same-named siblings"
---
We recommend avoiding multiple inputs with the same name at the same level of depth in your form tree. If this is unavoidable due to your template structure, the <code>mergeStrategy</code> config option can help keep their values synchronized.
::
...
label: "The form type"
---
The <code>&lt;FormKit type="form"&gt;</code> input already incorporates this await behavior. It will not call your :FrameworkText{vue="<code>@submit</code>" react="<code>onSubmit</code>"} handler until your form is completely settled. However when building advanced inputs it can be useful to understand these underlying principles.
::
...
- Using :FrameworkText{vue="the <code>@node</code> event" react="the <code>onNode</code> prop"}
- Using :FrameworkText{vue="a template <code>ref</code>" react="a React <code>ref</code>"}
...
---
  name: "Get core node"
  file: "_examples/node-get/node-get.vue"
  react-file: "_examples/node-get/node-get.react.jsx"
---
::
...
The `useFormKitNodeById` composable allows you to access a node by its `id` from within your component. :FrameworkText{vue="In Vue it returns a <code>ref</code> that is populated with the <code>FormKitNode</code> instance as soon as it has been created." react="In React you pass the input id and a callback that receives the <code>FormKitNode</code> as soon as it becomes available."}
...
---
  name: "Get core node"
  file: "_examples/use-formkit-node-by-id/my-form.vue"
  react-file: "_examples/form/use-formkit-node-by-id/my-form.react.jsx"
---
::

### Using the node event

Another way to get the underlying `node` is to listen to :FrameworkText{vue="the <code>@node</code> event" react="the <code>onNode</code> prop"}, which is emitted only once when the component first initializes the node.

::Example
---
  name: "Node event"
  file: "_examples/node-event/node-event.vue"
  react-file: "_examples/node-event/node-event.react.jsx"
---
::

### Using a ref

Assigning a `<FormKit>` component to :FrameworkText{vue="a template <code>ref</code>" react="a React <code>ref</code>"} also allows easy access to the node.

::Example
---
  name: "Node ref"
  file: "_examples/node-ref/node-ref.vue"
  react-file: "_examples/node-ref/node-ref.react.jsx"
---
::
...
Nodes have their own events which are emitted during the node’s lifecycle (unrelated to your framework component events).
...
---
  name: "Plugin library"
  file: "_examples/plugin-library/plugin-library.vue"
  react-file: "_examples/plugin-library/plugin-library.react.jsx"
  layout: "auto"
---
::
...
---
  name: "Count visible"
  file: "_examples/count-visible/count-visible.vue"
  react-file: "_examples/count-visible/count-visible.react.jsx"
---
::
