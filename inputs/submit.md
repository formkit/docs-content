...
Importantly the `ignore` prop is automatically set to `true` — meaning even if a submit is given an value, it will not communicate it with the parent form. However this default behavior can be changed by setting the prop `:ignore="false"`.
